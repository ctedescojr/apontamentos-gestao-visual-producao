...
def enviaEmail(etapa, numero, ordem, et):
    ...
    if str(etapa) == "2":
        ...
        mensagem = (
            f"A etapa 1, OS nº {numero} foi finalizada. Teste de pressão concluído.\n"
    ...
    if str(etapa) == "3":
        ...
        mensagem = (
            f"A etapa 3, OS nº {numero}  foi concluida. Serviço finalizado\n"
            f"Cliente: {ordem.cliente}\n"
            f"Modelo: {ordem.modelo}\n"
            f"Tipo de Serviço: {ordem.get_tipo_display()}\n\n"
            f"Observações Ordem: {ordem.obs_ordem}\n\n"
    ...
    if str(etapa) == "0":
        ...
        mensagem = (
            f"Essa OS é um retrabalho, os nº {numero}\n"
            f"Cliente: {ordem.cliente}\n"
            f"Modelo: {ordem.modelo}\n"
            f"Tipo de Serviço: {ordem.get_tipo_display()}\n\n"
    ...
    if str(etapa) == "1":
        ...
        mensagem = (
            f"Essa OS é uma garantia, os nº {numero}\n"
            f"Cliente: {ordem.cliente}\n"
            f"Modelo: {ordem.modelo}\n"
            f"Tipo de Serviço: {ordem.get_tipo_display()}\n\n"
            f"Observações: {ordem.obs_ordem}"
    ...
    if str(etapa) == "5":
        ...
        mensagem = (
            f"A etapa 2, OS nº {numero} foi finalizada, prossiga com os trâmites de valores.\n"
            f"Cliente: {ordem.cliente}\n"
            f"Modelo: {ordem.modelo}\n"
            f"Tipo de Serviço: {ordem.get_tipo_display()}\n\n"
            f"Observações Ordem: {ordem.obs_ordem}\n\n"
    ...
    if str(etapa) == "6":
        ...
        mensagem = (
            f"O radiador, OS nº {numero} foi finalizado, fabricação concluída.\n"
            f"Cliente: {ordem.cliente}\n"
            f"Modelo: {ordem.modelo}\n"
            f"Tipo de Serviço: {ordem.get_tipo_display()}\n\n"
            f"Observações Ordem: {ordem.obs_ordem}\n\n"
...
def atualisa_status(request):
    ...
    if dados.operador_id == 3:
        messages.warning(
            request, "Por favor, indique um operador para iniciar o trabalho."
        )
        if dados.fase == "2":
            dados.status = "6"
        elif dados.fase == "3":
    ...
    if dados.status == "0" and dados.fase == "1":
        ...
        id_ordem = dados.id_ordem_id
        final = Etapa.objects.filter(id_ordem_id=id_ordem, fase="2")
        ...
        if dados.rad_novo == "S":
            ...
            id_ordem = dados.id_ordem_id
            final = Etapa.objects.filter(id_ordem_id=id_ordem, fase="3")
    ...
    # Rotina de quando há uma parada na etapa 1
        ...
        messages.success(
            request,
            f"Etapa um retomada com sucesso após parada por motivo: {motivo_parada}.",
        )
        ...
        dados.decorrido = (
            calcula(dados.inicio, dados.fim) - dados.parado
        )  # Calcula o tempo
        dados.save()
        id_ordem = dados.id_ordem_id
        final = Ordem.objects.get(base_ptr_id=id_ordem)
        ...
        if dados.rad_novo == "S":
            ...
            messages.success(
                request, "Primeira etapa finalizada com sucesso após parada."
            )
            # enviaEmail('2', final.numeros,final, dados)
    ...
    # Rotina de quando há uma parada na etapa 2
        ...
        messages.success(
            request,
            f"Etapa dois retomada com sucesso após parada por motivo: {motivo_parada}.",
        )
    ...
    # Rotina de quando há uma parada na etapa 3
        ...
        messages.success(
            request,
            f"Etapa três retomada com sucesso após parada por motivo: {motivo_parada}.",
        )
...
def dashboard(request):
    ...
    result = Ordem.objects.values("tipo").annotate(dcount=Count("tipo")).order_by()
    ordem_by_operation = (
        Ordem.objects.values("tipo").annotate(dcount=Count("tipo")).order_by()
    )
